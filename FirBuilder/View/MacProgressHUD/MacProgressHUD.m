...
NSString *MacProgressHUDIdentifier = @"MacProgressHUDIdentifier";
...
+(void)customView {
    ...
    gloomyView.identifier = MacProgressHUDIdentifier;
...
+ (void)showWaitingWithTitle:(NSString *)title time:(NSInteger)showTime
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //修正hub的frame
        gloomyView.frame = NSApplication.sharedApplication.keyWindow.contentViewController.view.bounds;
        ...
        titleTF.frame = NSMakeRect(x, y, 60, 60);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (contentView) {
            for (NSView *sub in contentView.subviews) {
                if ([sub isKindOfClass:MacClickView.class]) {
                    [(MacClickView *)sub stopHUD];
                }
            }
        }
    });
...
//永久显示点点
+ (void)showWaitingWithPointColor:(NSColor *)color{
    [self showWaitingWithPointColor:color speed:2 time:1000000];
}
...
{
    animState = NO;
    [gloomyView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];

    [gloomyView removeFromSuperview];
    [quareViewsMAry removeAllObjects];
